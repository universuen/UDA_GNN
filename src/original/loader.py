...
class MoleculeDataset_aug_v2(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            ### 
            downstream_dir = [
                config.Paths.datasets / 'bace',
                config.Paths.datasets / 'bbbp',
                config.Paths.datasets / 'clintox',
                config.Paths.datasets / 'esol',
                config.Paths.datasets / 'freesolv',
                config.Paths.datasets / 'hiv',
                config.Paths.datasets / 'lipophilicity',
                config.Paths.datasets / 'muv',
                # 'dataset/pcba/processed/smiles.csv',
                config.Paths.datasets / 'sider',
                config.Paths.datasets / 'tox21',
                config.Paths.datasets / 'toxcast'
            ]
...
class MaskAtom2(object):
    ...
    def __call__(self, data, masked_atom_index=None):
        ...
        * mask rate number of atom indices
        ...
        # modify the original node feature of the masked node
        data.x[data.masked_atom_index] = torch.LongTensor([self.num_atom_type, 0]).view(1,
                                                                                        -1)  # .repeat(len(masked_atom_indices), 1)
...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            for d_path in downstream_dir:
                print(d_path)
                dataset_name = str(d_path).split('\\')[1]
                downstream_dataset = MoleculeDataset(d_path, dataset=dataset_name)
                downstream_smiles = pd.read_csv(os.path.join(d_path,
                                                             'processed', 'smiles.csv'),
                                                header=None)[0].tolist()
...
def create_all_datasets():
    ...
    for dataset_name in downstream_dir:
        print(dataset_name)
        root = config.Paths.datasets / dataset_name
        os.makedirs(root / "processed", exist_ok=True)
        dataset = MoleculeDataset(root, dataset=dataset_name)
        print(dataset)

    dataset = MoleculeDataset(root=str(config.Paths.datasets / "chembl_filtered"), dataset="chembl_filtered")
    print(dataset)
    dataset = MoleculeDataset(root=str(config.Paths.datasets / "zinc_standard_agent"), dataset="zinc_standard_agent")
    print(dataset)
